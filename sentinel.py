...
def email_sender():
    ''' Counts the number of pictures in the /tmp/motion' folder and if the number of pics
    is  geq 5, then we first send an email to the user with those pics attached, and we
    will move those pics to an archive folder, with the date and time of intrusion. We will
    also store the current date and time in a pickle file, and will click more pics only if 
    ...
    if image_count >= MAX_PICS_BEFORE_EMAIL and intrusion_time_delta >= datetime.timedelta(minutes=5):
        ...
        for p in pics:
            os.rename(pics_directory+'/'+p,  archive_directory+'/'+str(current_intrusion_datetime)+'/'+p)
